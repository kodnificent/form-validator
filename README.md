...
By default, FormValidator uses `'form'` as the Selector if no value is present.
You can use any valid element selector or an HTMLElement.

**Using Selector**

```javascript
const validator = new FormValidator('.my-form-class');
```
...
const validator = new FormValidator(el);
```

### Conifguration Options

The second paramater of the FormValidator constructor should be an object containing the following configuration options.
...
FormValidator ships with some core rules that contains most of the validation rules we apply regularly.
...
FormValidator is inspired by [Laravel's validation feature](https://laravel.com/docs/8.x/validation).
